...
const useParamPaymentDetails = ({ noLinkRedirection, enableToast, noLoginRedir }: { noLinkRedirection: boolean, enableToast: boolean, noLoginRedir?: boolean}) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const customFormatRegex = /^([0-9a-z]+)([SGPDFX])([0-9a-z]+)K([0-9a-z]+)M([0-9a-z]+)(GA|GF|GS|GO|GT|GAP|GAN|GM|GSHS|GSH)?$/;
      const serviceFormatRegex = /^S([a-n])K([0-9a-z]+)M([0-9a-z]+)(GA|GF|GS|GO|GT|GAP|GAN|GM|GSHS|GSH)?$/;
      ...
      if (matchService) {
        ...
        parsed = {
          ...
          gateway: matchService[4] === 'GA' ? 'Authorize.net' : matchService[4] === 'GO' ? 'Online Payment' : matchService[4] === 'GAN' ? 'Antom' : matchService[4] === 'GM' ? 'MOR' : matchService[4] === 'GSHS' ? 'Shopify Subscription' : matchService[4] === 'GSH' ? 'Shopify' : matchService[4] === 'GT' || matchService[4] === 'GS' ? 'Stripe' : 'FastSpring',
          time: Date.now()
        };
      } else if (match) {
        ...
        parsed = {
          ...
          gateway: match[6] === 'GA' ? 'Authorize.net' : match[6] === 'GO' ? 'Online Payment' : match[6] === 'GAN' ? 'Antom' : match[6] === 'GM' ? 'MOR' : match[6] === 'GSHS' ? 'Shopify Subscription' : match[6] === 'GSH' ? 'Shopify' : match[6] === 'GT' || match[6] === 'GS' ? 'Stripe' : 'FastSpring',
          time: Date.now()
        };

import { NextResponse } from 'next/server';
import { connectToDatabase } from '@/app/lib/mongodb';
import { createSubscriptionCheckoutUrl, PAYROLL_SUBSCRIPTION_VARIANTS } from '@/app/lib/shopify';

export async function POST(req: Request) {
  try {
    const body = await req.json();
    const {
      tier, email, firstName, lastName, phone, planDetails,
      address, city, state, zipCode, country, companyName, ein,
      clientSignatureBase64, agreedToTerms
    } = body;

    if (!(tier in PAYROLL_SUBSCRIPTION_VARIANTS)) {
      return NextResponse.json({ error: 'Invalid subscription tier' }, { status: 400 });
    }

    const forwarded = req.headers.get('x-forwarded-for');
    const ipAddress = forwarded ? forwarded.split(',')[0].trim() : req.headers.get('x-real-ip') || 'Unknown';
    const userAgent = req.headers.get('user-agent') || '';
    const deviceType = /mobile|android|iphone|ipad/i.test(userAgent) ? 'Mobile' : 'Desktop';
    const browserMatch = userAgent.match(/(chrome|firefox|safari|edge|opera)[\/\s][\d.]+/i);
    const browser = browserMatch ? browserMatch[0] : userAgent.substring(0, 60) || 'Unknown';

    const { db } = await connectToDatabase();

    const result = await db.collection('admindata').insertOne({
      firstName, lastName, email, phone, companyName, ein,
      address, city, state, zipCode, country,
      amountUSD: Number(tier),
      planDetails: planDetails || 'QuickBooks Payroll (Monthly Subscription)',
      status: 'Pending',
      agreedToTerms: agreedToTerms === 'true' || agreedToTerms === true,
      clientSignatureBase64,
      agreedTimestamp: new Date(),
      createdAt: new Date(),
      updatedAt: new Date(),
      paymentGateway: 'Shopify Subscription',
      isSubscription: true,
      subscriptionTier: tier,
      ipAddress,
      deviceType,
      browser
    });

    const localOrderId = result.insertedId.toString();

    const checkoutUrl = await createSubscriptionCheckoutUrl({ tier, localOrderId, email });

    if (!checkoutUrl) {
      console.error('[Shopify Subscribe Checkout] cartCreate returned no checkoutUrl');
      return NextResponse.json({ error: 'Shopify subscription checkout creation failed' }, { status: 502 });
    }

    return NextResponse.json({ redirectUrl: checkoutUrl, localOrderId });
  } catch (error: any) {
    console.error('[Shopify Subscribe Checkout Error]', error);
    return NextResponse.json({ error: 'Internal Server Error', details: error.message }, { status: 500 });
  }
}

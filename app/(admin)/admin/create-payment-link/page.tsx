...
import { ShieldCheck, FileText, RefreshCw, Layers, Link as LinkIcon, AlertCircle, Copy, Check, CheckCircle, ChevronDown, ChevronUp, ChevronLeft, ChevronRight, Filter, Eye, X, Search, Mail, MailWarning, Users, Upload, Download, MessageSquare } from 'lucide-react'
...
export default function QuickBooksPaymentLinkCreator() {
  ...
  // history:'replace' so clicking between tabs doesn't spam the browser back button.
  const [activeTab, setActiveTab] = useQueryState(
    'tab',
    parseAsStringLiteral(['create', 'logs', 'email', 'sentEmails', 'bulk'] as const)
      .withDefault('create')
    ...
    const fetchProvider = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setProviderParam(data.provider)
        }
      } catch {
        // Non-critical — the badge just won't show a value until this loads.
      }
    }
    fetchProvider()
  }, [])
  ...
  // On a hard reload, the `admin` atom resets to false (it's in-memory, not persisted) —
  // without this, every reload bounced through /admin, whose own login-check redirects
  // back with a hardcoded '/admin/create-payment-link' with no query string at all. That
  // silently wiped ?tab= (and ?provider=) on every reload regardless of nuqs, since the
  // page never got to keep its URL — it was navigated away and back. Checking the same
  // stored auth here instead means a valid session re-authenticates in place, so the URL
  // (and the tab it points to) never changes.
  useEffect(() => {
    if (admin) return
    try {
      const stored = localStorage.getItem('adminAuth')
      if (stored) {
        ...
        if (parsed.expires > new Date().getTime() && parsed.passwordHash && encoded && atob(parsed.passwordHash) === atob(encoded)) {
          setAdmin(true)
          return
        }
...
By making a payment to QB Enterprise, you acknowledge that you have read, understood, and agree to be bound by these Terms, including the no-refund and no-chargeback provisions and the liability for legal fees arising from a chargeback.`;
      ...
      if (log.paymentGateway === 'Online Payment' && log.paymentProofType !== 'pdf') {
        ...
        doc.text(`${log.paymentType === 'wire_ach' ? 'Wire/ACH' : log.paymentType === 'credit_card' ? 'Credit Card' : 'N/A'}`, 150, 38);
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center justify-between pb-6 mb-8 border-b border-zinc-200">
          ...
          <div className="w-full md:w-auto self-start md:self-auto overflow-x-auto no-scrollbar">
            <div className="inline-flex h-10 items-center justify-center rounded-lg bg-zinc-100 p-1 text-zinc-500 border border-zinc-200/50 font-medium">
              ...
              {/* Surveys lives on its own route (/admin/surveys), not a `tab` query-param
                  value like the others above — it has its own data-fetching lifecycle
                  (own table, own pagination) that doesn't fit this page's shared state. */}
              <NextLink
                href="/admin/surveys"
                className="inline-flex shrink-0 items-center justify-center gap-2 whitespace-nowrap rounded-md px-4 py-1.5 text-xs md:text-sm font-semibold transition-all cursor-pointer select-none hover:text-zinc-900 text-zinc-500"
              >
                <MessageSquare size={14} className="text-[#2ca01c]" />
                Surveys
              </NextLink>
            </div>
          </div>
            ...
            <form onSubmit={sendCustomEmail} className="bg-white border border-zinc-200 rounded-xl shadow-xs p-6 md:p-8 space-y-6">
              ...
              <div className="bg-zinc-50 rounded-lg p-4 border border-zinc-200">
                ...
                <div className="flex bg-white rounded-md border border-zinc-200 p-1">
                  ...
                  <button
                    type="button"
                    onClick={() => setEmailType('survey')}
                    className={`flex-1 text-xs font-semibold py-2 rounded transition-colors cursor-pointer ${emailType === 'survey' ? 'bg-indigo-600 text-white shadow-sm' : 'text-zinc-600 hover:bg-zinc-50'}`}
                  >
                    Survey
                  </button>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 border-t pt-5 border-zinc-100">
                ...
                <div>
                  <label className="block mb-1.5 font-medium text-xs text-zinc-500">
                    Company Name
                    {emailType === 'survey' && <span className="text-[10px] text-zinc-400 italic font-normal"> (included in the survey link and shown in the email)</span>}
                  </label>
                  <input
                    type="text"
              ...
              {emailType === 'success' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 border-t pt-5 border-zinc-100">
                  ...
                  <div>
                    <label className="block mb-1.5 font-medium text-xs text-zinc-500">Due Date <span className="text-zinc-300">(optional)</span></label>
                    <input
                      type="date"
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 border-t pt-5 border-zinc-100">
                  ...
                  <div>
                    <label className="block mb-1.5 font-medium text-xs text-zinc-500">Due Date *</label>
                    <input
                      type="date"
                      required
                      value={emailForm.dueDate}
